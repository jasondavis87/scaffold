...
- `main` is the trunk and is always deployable. Branch off `main`, PR back
  into `main`. Every change goes through a PR.
- Small atomic commits. Checks pass before every commit.
- Never push to `main` directly. Never merge a PR unless explicitly told to.
- Store submissions are tagged `ios/<version>+<build>` /
  `android/<version>+<build>`, annotated, after the build succeeds.
  See docs/WORKFLOW.md in the skills repo.
